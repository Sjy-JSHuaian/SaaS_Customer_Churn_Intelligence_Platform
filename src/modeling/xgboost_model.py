...
流程：
    ...
    2. 训练 XGBClassifier (scale_pos_weight 处理不平衡)
...
def train(X_train: np.ndarray, y_train: np.ndarray) -> XGBClassifier:
    ...
    使用 scale_pos_weight 处理类别不平衡 (~26.5% 正类)。
    """
    ...
    print(f"[XGB] Training complete: {X_train.shape[0]:,} samples")
...
def _plot_confusion_matrix(y_test: np.ndarray, y_pred: np.ndarray) -> None:
    ...
    ax.set_title("Confusion Matrix - XGBoost", fontweight="bold", fontsize=13)
    ax.set_xlabel("Predicted")
    ax.set_ylabel("Actual")
    fig.tight_layout()
    path = FIGURES_DIR / "xgb_confusion_matrix.png"
...
def _plot_roc_curve(y_test: np.ndarray, y_proba: np.ndarray, auc: float) -> None:
    ...
    fig, ax = plt.subplots(figsize=(6, 5))
    ax.plot(fpr, tpr, color="#2C3E50", linewidth=2.5, label=f"XGB (AUC = {auc:.4f})")
    ax.plot([0, 1], [0, 1], color="gray", linestyle="--", linewidth=1.2, label="Random")
    ax.set_xlabel("False Positive Rate")
    ax.set_ylabel("True Positive Rate")
    ax.set_title("ROC Curve - XGBoost", fontweight="bold", fontsize=13)
    ax.legend(loc="lower right")
    ax.set_xlim([-0.02, 1.02])
    ax.set_ylim([-0.02, 1.02])
    fig.tight_layout()
    path = FIGURES_DIR / "xgb_roc_curve.png"
    fig.savefig(path, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"  [OK] Saved: {path}")
...
def _plot_feature_importance(
    ...
    colors = sns.color_palette("magma", len(top_imp))
    bars = ax.barh(
    ...
    fig.tight_layout()
    path = FIGURES_DIR / "xgb_feature_importance.png"
    fig.savefig(path, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"  [OK] Saved: {path}")
...
def run() -> dict:
    ...
    print("=" * 60)
    print(f"  Model: models/xgboost.pkl")
